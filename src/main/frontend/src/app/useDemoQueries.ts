...
export function useDemoQueries(showLab = false) {
  ...
  const scenarios = useQuery({ queryKey: ["scenarios"], queryFn: api.scenarios, enabled: showLab });
  const kafka = useQuery({ queryKey: ["kafka"], queryFn: api.kafkaStatus, refetchInterval: 750, enabled: showLab });
  const kafkaOperations = useQuery({ queryKey: ["kafka-operations"], queryFn: api.kafkaOperations, refetchInterval: 1500, enabled: showLab });
