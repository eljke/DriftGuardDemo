...
На каждом calibration потоке запускаются `AGGRESSIVE`, `BALANCED` и `CONSERVATIVE`. Профиль с максимальной utility становится меткой calibration example. Ground truth используется для построения этих меток, но не передаётся selector во время hold-out выбора.
...
Признаки стандартизуются по calibration выборке. Adaptive strategy выбирает профиль взвешенным методом пяти ближайших соседей. Scenario id, параметры генератора, ожидаемый интервал drift и результаты detector не являются входами selector.
