...
Признаки стандартизуются по calibration выборке. Adaptive strategy выбирает профиль взвешенным методом пяти ближайших соседей. Selector передаётся в публичный `AdaptivePageHinkleyConfig` библиотеки DriftGuard; hold-out поток обрабатывается библиотечным `adaptive-page-hinkley`, а выбранные пороги и соответствующая emission policy применяются из detector state. Scenario id, параметры генератора, ожидаемый интервал drift и результаты detector не являются входами selector.
